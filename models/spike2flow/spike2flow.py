...
from models.spike2flow.update import BasicUpdateBlock
from models.spike2flow.extractor import BasicEncoder
from models.spike2flow.corr import CorrBlock
from models.spike2flow.utils import bilinear_sampler, coords_grid, upflow8
from models.spike2flow.warp_utils import get_occ_mask_bidirection, flow_warp
...
class Spike2Flow(nn.Module):
    def __init__(self):
        ...
        self.fnet = BasicEncoder(output_dim=216, norm_fn='instance', dropout=self.dropout)        
        self.cnet = BasicEncoder(output_dim=hdim+cdim, norm_fn='instance', dropout=self.dropout)
        self.update_block = BasicUpdateBlock(hidden_dim=hdim, input_dim=cdim)
    ...
    def forward(self, spks, iters=8, flow_init=None, upsample=True, test_mode=False, phm=False):
        ...
        spk0 = spks[:, :21].contiguous()
        spk1 = spks[:, 3:24].contiguous()
        spk2 = spks[:, 6:27].contiguous()
        spk3 = spks[:, 9:30].contiguous()
        ...
        corr_fn1 = CorrBlock(fmap0, fmap1, num_levels=self.corr_levels, radius=self.corr_radius)
        corr_fn2 = CorrBlock(fmap0, fmap2, num_levels=self.corr_levels, radius=self.corr_radius)
        corr_fn3 = CorrBlock(fmap0, fmap3, num_levels=self.corr_levels, radius=self.corr_radius)
