...
def mesh_grid(B, H, W, device='cuda'):
    # mesh grid
    x_base = torch.arange(0, W, device=device).repeat(B, H, 1)  # BHW
    y_base = torch.arange(0, H, device=device).repeat(B, W, 1).transpose(1, 2)  # BHW

    base_grid = torch.stack([x_base, y_base], 1)  # B2HW
    return base_grid


def norm_grid(v_grid, device='cuda'):
    _, _, H, W = v_grid.size()

    # scale grid to [-1,1]
    v_grid_norm = torch.zeros_like(v_grid, device=device)
...
def flow_warp(x, flow12, pad='border', mode='bilinear'):
    ...
    base_grid = mesh_grid(B, H, W, device=x.device).type_as(x)  # B2HW

    v_grid = norm_grid(base_grid + flow12, device=x.device)  # BHW2
